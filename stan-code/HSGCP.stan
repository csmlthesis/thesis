...
parameters {
  ...
  matrix[5,J] country_gp_par_scaled_deviations ;   //non-centered std of length scale
...
transformed parameters {
  ...
  vector[J] country_site_mean = exp(log(site_m_mu) + site_s_mu * country_gp_par_scaled_deviations[4]');

  vector[J] country_site_sd = exp(log(site_m_sd) + site_s_sd * country_gp_par_scaled_deviations[5]'); 
...
model {
  ...
  for (j in 1:J) {
    target += poisson_lpmf(
      y[j,1:N[j]] | site_lambda[site[j,1:N[j]]] .* inv_logit(country_a[j] + log_f[j, n_tilde+1:n_tilde+N[j]]'));
  }
}
...
generated quantities {  
  ...
  for (j in 1:J) {
    a_plus_f[j,1:n_tilde] = inv_logit(country_a[j] + log_f[j,1:n_tilde]);
  }
  ...
  for (j in 1:J) {
      for (n in 1:N[j]){
          ...
          log_lik[count] = poisson_lpmf(y[j,n] | site_lambda[site[j, n]] *
                                        inv_logit(country_a[j] + log_f[j,n_tilde+n]));
      }
  }
}