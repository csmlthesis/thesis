...
model {    
  ...
  for (j in 1:J) {
    target += poisson_lpmf(
      y[j,1:N[j]] | exp(country_a[j] + country_b[j] * to_vector(log(x[j,1:N[j]])) + log_f[j, n_tilde+1:n_tilde+N[j]]'));
  }
}
generated quantities{
  ...
  for (j in 1:J) {
    a_plus_f[j,1:n_tilde] = exp(country_a[j] + country_b[j] * to_row_vector(log(x_tilde[j,1:n_tilde])) + log_f[j,1:n_tilde]);
  }
  ...
  for (j in 1:J) {
      for (n in 1:N[j]){
          count += 1;
          log_lik[count] = poisson_lpmf(y[j,n] | exp(country_a[j] + country_b[j] * log(x[j,n])+ 
                                                     log_f[j,n_tilde+n]));
      }
  }
}